...
/*
 * Wrapper for dispatching through the shared executor.
 * Adapts the generic callback signature to scalar function's callback_arg.
 */
static void scalar_execute_via_executor(void *data) {
 ...
 *   3. Non-Ruby thread         -> use per-worker proxy (or fallback to global executor)
 */
static void scalar_function_callback(duckdb_function_info info, duckdb_data_chunk input, duckdb_vector output) {
    rubyDuckDBScalarFunction *ctx;
    idx_t i;
    ...
    if (ruby_native_thread_p()) {
        ...
        /* Case 3: Non-Ruby thread - use per-worker proxy if available */
